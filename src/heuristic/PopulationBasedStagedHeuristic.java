...
public final class PopulationBasedStagedHeuristic extends HeuristicAdapter implements StatisticPrinter {
	...
	protected void allocateSolutions(final SolutionAllocator allocator) {
		...
		for (final var mutator : mutators) mutator.allocateBufferSolutions(allocator);
	}
...
//		final var solutionChooser = new RandomSelector(population, rng);
	...
	protected void iterate() {
		...
		val mutator = mutators[Utils.binarySearchLess(thresholds, withoutImprovement[solutionIndex])];
...
//			localSearches[9].apply(solution,solution);
	...
	public void printStats(final NestedWriter output) {
		...
		scoped.formatLine("thresholds: %s", Arrays.toString(thresholds));
		scoped.printLine("mutators: [");
		{
			val newScoped = scoped.getScoped();
			newScoped.indent();
			for (final var mutator : mutators) mutator.printStats(newScoped);
		}
